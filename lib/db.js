...
export async function insertNewIocs(newIocs, batchId) {
  ...
  // Serialize each tags array to a PostgreSQL text[] literal string so Neon's
  // tagged-template client passes it as a plain string — avoids the
  // "malformed array literal" error caused by JS array-of-arrays being
  // serialized as text[][] with double-braces {{"a","b"}} which PG rejects.
  const tagLiterals = newIocs.map((i) => {
    const arr = i.tags || [];
    // Escape double-quotes inside each tag value, wrap whole array in {}.
    const escaped = arr.map((t) => `"${String(t).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`);
    return `{${escaped.join(',')}}`;
  });
...
export async function listIocs(filters = {}) {
  ...
  // Build the query dynamically. neon() can be called as sql(text, params)
  // (plain function call) for parameterized queries — it returns Row[] directly.
  // Do NOT use sql.query() — that method does not exist on the neon client.
  const conditions = [];
  const params = [];
  ...
  // sql.query(text, params) returns Row[] directly — NOT { rows }.
  // Confirmed by test: Array.isArray(result)===true, result.rows===undefined.
  // Do NOT do `result.rows` or you get undefined → iocs.map() crash in browser.
