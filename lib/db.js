...
// Required for @neondatabase/serverless when running in Node.js (Vercel
// serverless functions). Without this, the driver cannot open the WebSocket
// connection it uses instead of TCP, and every query silently fails.
...
function getPool() {
  if (!pool) {
    ...
    // Do NOT pass `ssl` manually — Neon reads sslmode from the connection
    // string. Passing ssl:{rejectUnauthorized:false} can conflict with
...
export async function insertNewIocs(newIocs, batchId) {
  ...
  // Use pool.query() directly — @neondatabase/serverless manages HTTP-level
  // connection multiplexing, so we don't need to acquire/release clients.
